...
void seta_gpidLst(tLista* lst, pid_t gpid) {    //setando todos os processos do tipo Vacinados no mesmo grupo.
    lst->gpidVacinados = gpid;
}

int ehSaidaRedirecionada(char** comando, int nArgs) {       //tratando o direcionamento a saída padrão de um processo não vacinado para um arquivo.
...
void executaNaoVacinados(tLista* lst) {
    ...
    if (procEhFilho(procGerente)) {
        ...
        setpgid(0,getpid());                      //criando um novo grupo para cada comando do tipo Não Vacinado.
        pid_t pidCmd;
        int estadoExec;
